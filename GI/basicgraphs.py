...
class vertex():
    ...
    def inclist(self):
        ...
        """
        incl=[]
        index = self._graph.findFirstEdgeOfVertex(self._label)
        while True:
            edge = self._graph.E()[index]
            if edge._tail._label == self._label:
                incl.append(edge)
                if index < len(self._graph.E()) - 1:
                    index += 1
                else:
                    return incl
            else:
...
class graph():
    ...
    def addedge(self,tail,head):
        ...
        """
        if tail.get_label() > head.get_label(): # nessesary for sorting but makes graph undirected by default
            tail, head = head, tail
        if self._simple:
            if tail==head:
    ...
    def sortEdgesRec(self, list):
        if len(list) > 1:
            ...
            while fi < len(f) and si < len(s):
                if f[fi].head().get_label() < s[si].head().get_label() or f[fi].head().get_label() == s[si].head().get_label() and f[fi].tail().get_label() < s[si].tail().get_label():
                    r.append(f[fi])
                    fi += 1
                else:
                    r.append(s[si])
    ...
    def findFirstEdgeOfVertex(self, label):
        if self.unsorted == True:
            self.sortEdges()
        return self.binary_search_pairs(self._E, label)
    ...
    def findedge(self,u,v):
        if self.unsorted == True:
            self.sortEdges()
        return self.findedgeRec(u, v, len(self._E), 0)
